...
namespace shenlan
{
  namespace control
  {
    MpcOsqp::MpcOsqp(const Eigen::MatrixXd &matrix_a,
        ...
        : matrix_a_(matrix_a),                 // 6 * 6 状态矩阵
          matrix_b_(matrix_b),                 // 6 * 2  控制矩阵
          matrix_q_(matrix_q),                 // 6 * 6  状态权重
          matrix_r_(matrix_r),                 // 2 * 2  控制权重
          matrix_initial_x_(matrix_initial_x), // 6 * 1   初始值
          ...
          matrix_x_ref_(matrix_x_ref),         // 6 * 1  参考序列
          ...
          eps_abs_(eps_abs)
    {
      ...
      num_param_ = state_dim_ * (horizon_ + 1) + control_dim_ * horizon_; // 6 * (10 + 1) + 2 * 10=88;
    }

    // 标准二次性中的P = diag(Q,Q,....Q, R, R,...R)
    //  1/2 *xT*P*x + qT*x
    //
                                  ...
                                  std::vector<c_int> *P_indptr)
    {
      ...
      std::vector<std::vector<std::pair<c_int, c_float>>> columns; // 二维矩阵
      ...
      for (size_t i = 0; i < horizon_; ++i)
      {
        for (size_t j = 0; j < control_dim_; ++j)
        {
          // (row, val)
          // 行索引, data值
          columns[i * control_dim_ + j + state_total_dim].emplace_back(state_total_dim + i * control_dim_ + j, matrix_r_(j, j));
          ++value_index;
        }
      }
...
Matrix Ac structure:
    ...
    [  .   .    .     .     .   |   .   .    .    . ]
    [  0   0    0    ...    I   |   0   0    ...   0 ]
  6个初始+(6个状态）*2个状态等式*10+2个控制*1个等式*10=152
  11个*6+10*2=86
Legend:
- I: Identity matrix
- A: A given matrix
- B: A given matrix
- 0: Zero matrix
- The structure is block-based, repeated along the diagonal and off-diagonal as shown.
*/
                                              ...
                                              std::vector<c_int> *A_indptr)
    {
      ...
      Eigen::MatrixXd matrix_constraint = Eigen::MatrixXd::Zero(
          ...
          state_dim_ * (horizon_ + 1) + control_dim_ * horizon_); // 152X86
      Eigen::MatrixXd state_identity_mat = Eigen::MatrixXd::Identity(state_dim_ * (horizon_ + 1),
                                                                     state_dim_ * (horizon_ + 1)); // 66X66
      ...
      for (size_t i = 0; i < horizon_; i++)
      {
        matrix_constraint.block((i + 1) * state_dim_, i * state_dim_, state_dim_, state_dim_) = matrix_a_; // 状态矩阵 60X54(最后一个)
      }

      for (size_t i = 0; i < horizon_; i++)
      {
        matrix_constraint.block((i + 1) * state_dim_, i * control_dim_ + (horizon_ + 1) * state_dim_, state_dim_, control_dim_) = matrix_b_; // 控制矩阵 60
      }
      Eigen::MatrixXd all_identity_mat = Eigen::MatrixXd::Identity(num_param_, num_param_); // 88X88个对角矩阵

      matrix_constraint.block(state_dim_ * (horizon_ + 1), 0, num_param_, num_param_) = all_identity_mat; // 66
...
*/
    ...
    void MpcOsqp::CalculateConstraintVectors()
    {
      ...
      Eigen::VectorXd upperInequality = Eigen::MatrixXd::Zero( // 决策变量上界 88X1
          state_dim_ * (horizon_ + 1) + control_dim_ * horizon_, 1);

      for (size_t i = 0; i < horizon_; i++)
      { // 控制变量上下界
        lowerInequality.block(control_dim_ * i + state_dim_ * (horizon_ + 1), 0, control_dim_, 1) = matrix_u_lower_;
        upperInequality.block(control_dim_ * i + state_dim_ * (horizon_ + 1), 0, control_dim_, 1) = matrix_u_upper_; // 2Xi+66
      }
      for (size_t i = 0; i < horizon_ + 1; i++)
      {                                                                            // 状态变量上下界
        lowerInequality.block(state_dim_ * i, 0, state_dim_, 1) = matrix_x_lower_; // 60
        upperInequality.block(state_dim_ * i, 0, state_dim_, 1) = matrix_x_upper_;
      }

      // evaluate the lower and the upper equality vectors
      // 等式约束
      Eigen::VectorXd lowerEquality = Eigen::MatrixXd::Zero(state_dim_ * (horizon_ + 1), 1); // 66X1
      ...
      upperBound_ = Eigen::MatrixXd::Zero(2 * state_dim_ * (horizon_ + 1) + control_dim_ * horizon_, 1); // 152X1
    ...
    OSQPData *MpcOsqp::Data()
    {
      ...
      else
      {
        ...
        data->P = csc_matrix(kernel_dim, kernel_dim, P_data.size(), CopyData(P_data),
                             CopyData(P_indices), CopyData(P_indptr)); // 权重矩阵
        data->q = gradient_.data();                                    // 损失函数
